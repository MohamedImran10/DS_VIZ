...
export default function ControlPanel({
  ...
  return (
    <section className="rounded-3xl border border-white/10 bg-white/6 p-5 shadow-glow backdrop-blur-xl">
      ...
      <div className="mt-5 grid gap-4 grid-cols-1 md:grid-cols-[1.6fr_1fr]">
        <div className="rounded-2xl border border-white/10 bg-slate-950/50 p-4">
          <label className="mb-2 block text-sm font-medium text-slate-100">Value to insert, delete, or search</label>
          <input
            value={value}
            ...
            className="w-full rounded-2xl border border-white/10 bg-slate-900/80 px-4 py-3 text-lg text-white outline-none transition placeholder:text-slate-300 focus:border-amber-300/70 focus:ring-2 focus:ring-amber-400/20"
          />
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            <button
              type="button"
              onClick={onInsert}
              className="w-full sm:inline-flex items-center justify-center sm:justify-start gap-2 rounded-2xl bg-gradient-to-r from-amber-400 to-orange-500 px-4 py-3 font-medium text-slate-950 shadow-lg shadow-amber-500/20 transition hover:brightness-110"
            >
            ...
            <button
              type="button"
              onClick={onDelete}
              className="w-full sm:inline-flex items-center justify-center sm:justify-start gap-2 rounded-2xl border border-rose-400/20 bg-rose-500/10 px-4 py-3 font-medium text-rose-100 transition hover:bg-rose-500/20"
            >
              <Trash2 size={18} />
              Delete
            </button>
            <button
              type="button"
              onClick={onSearch}
              className="w-full sm:inline-flex items-center justify-center sm:justify-start gap-2 rounded-2xl border border-cyan-400/20 bg-cyan-500/10 px-4 py-3 font-medium text-cyan-100 transition hover:bg-cyan-500/20"
            >
              <Search size={18} />
              Search
            </button>
        ...
        <div className="grid gap-4 rounded-2xl border border-white/10 bg-slate-950/50 p-4">
          <label className="block text-sm font-medium text-slate-100">Structure size limit</label>
